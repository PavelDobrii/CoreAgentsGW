...
3. Создайте файл окружения `city_guide/.env` (в репозитории есть шаблон `city_guide/.env.example`):
   ```bash
   cp .env.example .env
   ```
...
3. **ENV файл**. Убедитесь, что `.env` располагается в `city_guide/.env`. Переменная `CITY_GUIDE_ENV` не требуется — путь определяется автоматически.
...
- **PostgreSQL (рекомендуется для продакшена)**:
  1. Укажите строку подключения в `city_guide/.env`. Для локального PostgreSQL на WSL подойдет, например,
     ```dotenv
