...
import { CalendarDays, ChevronLeft, Map, Play } from 'lucide-react';
import {
    ActionIcon, Box,
    Button, Card,
    Flex,
    ...
    return (
        <Stack>
            <Flex align="start" gap="sm">
                <Image
                    src={data.startWaypoint.posterSrc}
                    fallbackSrc="https://placehold.co/84x56"
                    alt={data.name}
                    h={56}
                    w={84}
                    radius="md"
                ...
                <Stack gap={0} style={{ flex: 1, minWidth: 0 }}>
                    <Text lineClamp={1} fw={600} size="lg" truncate="end" p={0}>{data.name}</Text>
                    ...
                    return (
                        <Timeline.Item key={point.id} bullet={bullet}>
                            <Card
                                ...
                                style={style}
                            >
                                <Card.Section pos="relative" display="flex">
                                    {!isActive && (
                                        <Box pos="absolute" c="white" left="12px" top="30px" opacity="0.8">
                                            <Play size={40} absoluteStrokeWidth />
                                        </Box>
                                    )}

                                    <Image
                                        src={point.posterSrc}
                                        fallbackSrc="https://placehold.co/64x100"
                                        alt={point.name}
                                        h={100}
                                        w={64}
                                        fit="cover"
                                    ...
                                    <Stack p="xs" gap={4} flex={1} miw={0}>
                                        <Text size="sm" fw={600} lineClamp={1}>{point.name}</Text>
                                        <Text size="xs" c="dimmed" lineClamp={1}>{point.address}</Text>
                                        <Text
                                            size="xs"
                                            style={{ color: 'var(--mantine-color-text)', opacity: 0.75 }}
                                            lineClamp={2}
                                        >
                                            {point.description}
                                        </Text>
